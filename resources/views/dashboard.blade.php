        ...
        <div class="flex h-screen bg-gray-100">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
                ...
                <nav class="flex-1 px-2 py-4 space-y-2 overflow-y-hidden hover:overflow-y-auto">
                    @can('manage-personal')
                    <x-dropdown.simple.option class="w-full" :isActive="request()->routeIs('admin.*')">
                        <x-slot name="header">
                            <x-icons.director />
                            <span>{{ __('Personal') }}</span>
                        </x-slot>
                        <x-slot name="content">
                            <x-dropdown.simple.link :href="route('admin.index')">
                                {{ __('Lista del Personal') }}
                            </x-dropdown.simple.link>
                            <x-dropdown.simple.link :href="route('admin.create')">
                    ...
                    <x-dropdown.simple.option class="w-full" :isActive="request()->routeIs('contrato.*')">
                        <x-slot name="header">
                            <x-icons.report/>
                            <span>{{__("Contratos")}}</span>
                        </x-slot>
                        <x-slot name="content">
                            <x-dropdown.simple.link :href="route('contrato.index')">
                                {{ __('Lista de Contratos') }}
                            </x-dropdown.simple.link>
                            <x-dropdown.simple.link :href="route('contrato.create')">
                                {{ __('Crear un nuevo Contrato') }}
                            </x-dropdown.simple.link>
                        </x-slot>
                    </x-dropdown.simple.option>
                    @endcan
